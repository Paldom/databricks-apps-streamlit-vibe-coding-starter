...
## Claude Code Local Setup In This Repo

- Rules are in `.claude/rules/` (common + python).
- Reusable slash commands are in `.claude/commands/` (`/verify`, `/python-review`, `/new-streamlit-page`).
- Hook config is in `.claude/settings.json` and scripts in `.claude/hooks/`.
- Keep `.claude/settings.local.json` for machine/user-local permissions only.
